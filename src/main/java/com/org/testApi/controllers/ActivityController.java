...
@PostMapping@Operation(summary = "Créer une nouvelle activité", description = "Crée une nouvelle activité avec les données fournies")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Activité créée avecsuccès",
                    content= {@Content(mediaType= "application/json",
                            schema = @Schema(implementation = Activity.class))}),
...
activity.setDeleted(payload.getDeleted() != null? payload.getDeleted() : false);
            ...
            if (payload.getAssociationId() != null) {
                Association association = associationService.getAssociationById(payload.getAssociationId())
                       .orElseThrow(() -> new RuntimeException("Association not foundwith id: " + payload.getAssociationId()));
    ...
    @ApiResponses(value ={
           @ApiResponse(responseCode = "200",description = "Activité mise à jour avec succès",
                    content = {@Content(mediaType = "application/json",
                            schema = @Schema(implementation = ActivityResponseDTO.class))}),
            @ApiResponse(responseCode = "404", description = "Activiténon trouvée"),
            @ApiResponse(responseCode = "400", description = "Données de requête invalides"),
...
@Parameter(description = "ID de l'activité à mettre à jour") @PathVariable Long id,
                                            ...
                                            value = """
                                                    {
                                                      ...
                                                      "description": "Uneconférence annuelle sur la protection de l'environnement (mise à jour)",
                                                      "type": "CONFERENCE",
                                                      "startDateTime": "2025-11-20T09:00:00",
                                                      "endDateTime":"2025-11-20T18:00:00",
                                                      "location": "Centre de conférences de Marseille",
        ...
        try {
            ...
            e.printStackTrace();
            return ResponseEntity.status(500).body("Unexpectederror updating activity: " + e.getMessage());
        }
    }
    ...
    @ApiResponses(value = {
         @ApiResponse(responseCode = "200", description = "Activité mise à jour avec succès à partir du payload",
                    content = {@Content(mediaType = "application/json",
                            schema = @Schema(implementation = Activity.class))}),
            @ApiResponse(responseCode = "404", description = "Activiténontrouvée"),
    ...
    public ResponseEntity<?>updateActivityWithPayload(
            ...
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    ...
                    content = @Content(
                            ...
                            examples = {
                                    @ExampleObject(
                                            name = "Exemple demise à jour depayload d'activité",
...
try {
            ...
            // This is important because the association field isrequired (nullable = false)
            
            // Update projectif projectId is provided in payload
...
activityService.deleteActivity(id);
    ...
    @Operation(summary = "Lister les activités d'un projet", description = "Récupère la listedesactivités d'un projet spécifique")
    @ApiResponses(value = {
...
schema = @Schema(implementation = ActivityResponseDTO.class))}),
  ...
  @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Participantssupprimés avec succès",
                    content = {@Content(mediaType = "application/json",
                            schema = @Schema(implementation = ActivityResponseDTO.class))}),
            @ApiResponse(responseCode = "404",description = "Activité nontrouvée"),
            @ApiResponse(responseCode = "400", description ="Données de requête invalides"),
            @ApiResponse(responseCode = "500", description = "Erreur interne du serveur")
    })
    public ResponseEntity<?> removeParticipants(
            @Parameter(description = "ID de l'activité") @PathVariable Long id,
            @Parameter(description = "Liste desIDs des utilisateurs à supprimer des participants") @RequestBody List<Long> userIds) {
        try {
           ...
           // Convert to DTO to avoid serialization issues with Hibernate lazy loading
            ...
            e.printStackTrace();
            return ResponseEntity.status(500).body("Error removingparticipants: " + e.getMessage());
        }
    }
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description ="Liste des participantsrécupérée avec succès",
                    content = {@Content(mediaType = "application/json",
